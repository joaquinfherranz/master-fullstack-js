...
const dataHandler = (() => {
  ...
  const searchGitHubData = async (url, callback) => {
  ...
  const getOnLineData = (url, cacheData, callback) => 
    cacheData && areFreshCacheData(cacheData.time) ? callback(cacheData.data) : searchData(url, callback)
  const searchData = (url, callback) => isGithubRequest(url) ? searchGitHubData(url, callback) : getData(url, callback)
